...
def get_argparser():
    ...
    parser.add_argument('--config',default='configs/sample/cifar10/pre/resnet18_from_resnet50_origin.yaml',help='yaml file path')
    # densenet100_from_densenet250-final_run.yaml resnet18_from_resnet50-final_run.yaml
    parser.add_argument('--device', default='cuda', help='device')
    parser.add_argument('--log', default='log/cifar10/pre/resnet18_from_resnet50_origin_1.txt',help='log file path')
...
def train_one_epoch(training_box, device, epoch, log_freq):
    ...
    for sample_batch, targets, supp_dict in \
        ...
        model_outputs = training_box.model_forward_proc(training_box.model, sample_batch, targets, supp_dict)
        teacher_outputs = None
        org_loss_dict = training_box.extract_org_loss(training_box.org_criterion, model_outputs, teacher_outputs, targets,
        ...
        org_loss_dict[0].backward(retain_graph=True)
        org_loss_dict[1].backward(retain_graph=True)
        org_loss_dict[2].backward(retain_graph=True)
        training_box.optimizer1.step()
        training_box.optimizer2.step()
        training_box.optimizer3.step()
        loss= org_loss_dict[0]+org_loss_dict[1]+org_loss_dict[2]
...
def train(teacher_model, student_model, dataset_dict, ckpt_file_path, device, device_ids, distributed, config, args):
    ...
    optimizer1=torch.optim.SGD(training_box.model.layer1_fc.parameters(),1e-2)
    optimizer2=torch.optim.SGD(training_box.model.layer2_fc.parameters(),1e-2)
    optimizer3=torch.optim.SGD(training_box.model.layer3_fc.parameters(),1e-2)
    scheduler1=torch.optim.lr_scheduler.MultiStepLR(optimizer1,[50,75],0.1)
    scheduler2=torch.optim.lr_scheduler.MultiStepLR(optimizer2,[50,75],0.1)
    scheduler3=torch.optim.lr_scheduler.MultiStepLR(optimizer3,[50,75],0.1)
