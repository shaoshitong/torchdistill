...
class AuxICPLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, target,*args, **kwargs):
        ...
        target_identity,target_classes,target_policy=F.one_hot(target[:,0],output_identity.shape[1]).cuda(),F.one_hot(target[:,1],output_classes.shape[1]).cuda(),F.one_hot(target[:,2],output_policy.shape[1]).cuda()
        target_identity, target_classes, target_policy=target_identity.float(),target_classes.float(),target_policy.float()
        ...
        policy_loss=self.policy_kl_loss(output_policy,target_policy,target_policy)
        return identity_loss+classes_loss+policy_loss

@register_single_loss
class ICPLoss(nn.Module):
    def __init__(self, student_linear_module_path, teacher_linear_module_path, student_policy_module_path,teacher_policy_module_path, kl_temp,
                 ...
                 loss_weights=None,kd_and_ce_weight=[1,1],negative_loss_weight = [1.0, 0.5, 0.001], positive_loss_weight = [1.0, 0.5, 0.001],temperature = 1.0, ** kwargs):
        super().__init__()
    ...
    def icp_loss(self,teacher_output,student_output,targets):
        ...
        target_identity,target_classes,target_policy=F.one_hot(targets[:,0],student_output_identity.shape[1]).cuda(),F.one_hot(targets[:,1],student_output_classes.shape[1]).cuda(),F.one_hot(targets[:,2],student_output_policy.shape[1]).cuda()
        target_identity, target_classes, target_policy=target_identity.float(),target_classes.float(),target_policy.float()
        with torch.no_grad():
            teacher_output_identity, teacher_output_classes, teacher_output_policy = teacher_output
        """======================================================KD============================================"""
        kl_identity_loss=self.identity_kl_loss(student_output_identity,teacher_output_identity,target_identity)
        kl_classes_loss=self.classes_kl_loss(student_output_classes,teacher_output_classes,target_classes)
        kl_policy_loss=self.policy_kl_loss(student_output_policy,teacher_output_policy,target_policy)
        ...
        ce_policy_loss=self.policy_ce_loss(student_output_policy,target_policy,target_policy)
        ce_loss=ce_identity_loss + ce_classes_loss + ce_policy_loss
        if self.iter%1000==0:
            print(kl_identity_loss.item(),kl_classes_loss.item(),kl_policy_loss.item(),ce_identity_loss.item(),ce_classes_loss.item(),ce_policy_loss.item())
            self.iter=0
        self.iter+=1
        total_loss=0.
